...
int verify_callback(int preverify_ok, X509_STORE_CTX *ctx) {
  if (!preverify_ok) {
    ...
    static std::mutex warning_mutex;
    {
      ...
      static std::unordered_map<string, double> next_warning_time;
...
Result<SslCtxPtr> do_create_ssl_ctx(SslCtx::Options ctx_options) {
  auto ssl_method =
#if OPENSSL_VERSION_NUMBER >= 0x10100000L
      ctx_options.mode == SslCtx::Mode::Client ? TLS_client_method() : TLS_server_method();
#else
      ctx_options.mode == SslCtx::Mode::Client ? SSLv23_client_method() : SSLv23_server_method();
#endif
  ...
  if (ctx_options.cert_file.empty()) {
    ...
    if (SSL_CTX_load_verify_locations(ssl_ctx, ctx_options.cert_file.c_str(), nullptr) == 0) {
...
Result<SslCtxPtr> get_default_ssl_ctx() {
  static auto ctx = do_create_ssl_ctx(SslCtx::Options{CSlice(), SslCtx::VerifyPeer::On});
  if (ctx.is_error()) {
...
Result<SslCtxPtr> get_default_unverified_ssl_ctx() {
  static auto ctx = do_create_ssl_ctx(SslCtx::Options{CSlice(), SslCtx::VerifyPeer::Off});
  if (ctx.is_error()) {
    return ctx.error().clone();
  }
...
class SslCtxImpl {
 public:
  Status init(const SslCtx::Options &options) {
    SslCtx::init_openssl();

    clear_openssl_errors("Before SslCtx::init");

    if (options.cert_file.empty() && options.mode == SslCtx::Mode::Client) {
      if (options.verify_peer == SslCtx::VerifyPeer::On) {
    ...
    auto r_ssl_ctx_ptr = do_create_ssl_ctx(options);
  ...
  void init(SSL_CTX *ctx) {
    ssl_ctx_ptr_ = SslCtxPtr(ctx, SSL_CTX_free);
  }
...
Result<SslCtx> SslCtx::create(CSlice cert_file, VerifyPeer verify_peer) {
  return create(Options{cert_file, verify_peer});
}

Result<SslCtx> SslCtx::create(Options options) {
  auto impl = make_unique<detail::SslCtxImpl>();
  TRY_STATUS(impl->init(options));
...
Result<SslCtx> SslCtx::create(void *holder) {
  auto impl = make_unique<detail::SslCtxImpl>();
  impl->init(reinterpret_cast<SSL_CTX *>(holder));
  return SslCtx(std::move(impl));
}
