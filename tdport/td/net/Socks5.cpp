...
void Socks5::send_greeting() {
  ...
  sfd_.output_buffer().append(greeting);
}

Status Socks5::wait_greeting_response() {
  auto &buf = sfd_.input_buffer();
...
Status Socks5::send_username_password() {
  ...
  request += password_;
  sfd_.output_buffer().append(request);
...
Status Socks5::wait_ip_address_response() {
  ...
  auto it = sfd_.input_buffer().clone();
  ...
  it.advance(2);
  sfd_.input_buffer().advance(total_size);
  state_ = State::Done;
  return Status::OK();
}

Result<Socks5::Action> Socks5::loop_impl() {
  switch (state_) {
  ...
  return state_ == State::Done ? Action::Finish : Action::KeepRunning;
}
