...
class SslStreamImpl {
 public:
  Status init_server(SslCtx ssl_ctx) {
    if (!ssl_ctx) {
    ...
    SSL_set_accept_state(ssl_handle.get());
...
Result<SslStream> SslStream::create_server(SslCtx ssl_ctx) {
  auto impl = make_unique<detail::SslStreamImpl>();
  TRY_STATUS(impl->init_server(ssl_ctx));
  return SslStream(std::move(impl));
}
...
SslStreamHelper::SslStreamHelper(ChainBufferReader &reader, ChainBufferWriter &writer, SslStream ssl_stream)
    : read_source_(&reader), write_sink_(&writer), ssl_stream_(std::move(ssl_stream)) {
  if (ssl_stream_) {
...
td::Status SslStreamHelper::read_loop() {
  if (ssl_stream_) {
  ...
  if (read_sink_.status().is_error()) {
    return std::move(read_sink_.status());
  }
  return Status::OK();
}
td::Status SslStreamHelper::write_loop() {
  write_source_.wakeup();
  if (write_sink_.status().is_error()) {
    return std::move(write_sink_.status());
  }
  return Status::OK();
}
td::Status SslStreamHelper::loop() {
  TRY_STATUS(read_loop());
  TRY_STATUS(write_loop());
  return Status::OK();
}
ChainBufferReader &SslStreamHelper::input_buffer() {
  return *read_sink_.get_output();
}
ChainBufferWriter &SslStreamHelper::output_buffer() {
