...
void gen_to_json(StringBuilder &sb, const tl::simple::Schema &schema, bool is_header) {
  ...
  if (is_header) {
    sb << "inline void to_json(JsonValueScope &jv, const cocoon::" << tl_name
       << "::Object &object) {\n"
          "  cocoon::"
       << tl_name << "::downcast_call(const_cast<cocoon::" << tl_name
       << "::Object &>(object),[&jv](const auto &object) { "
          "to_json(jv, object); });\n"
       << "}\n";
    sb << "inline void to_json(JsonValueScope &jv, const cocoon::" << tl_name << "::Function &object) {\n"
       << "  cocoon::" << tl_name << "::downcast_call(const_cast<cocoon::" << tl_name
