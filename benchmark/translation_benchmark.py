    ...
    def process_chunk(chunk_id, chunk, use_submitted=False, submit_time=0.0):
        ...
        if result.success:
            if chunk_id < 3 or (chunk_id + 1) % 20 == 0:
                print(f"[{chunk_id + 1}/{len(chunks)}] ✓ {result.duration:.2f}s{pending_str} | {chunk_len} chars | {speed:.0f} chars/s | active: {active} | queue: {queue_size}")
            ...
            print(f"[{chunk_id + 1}/{len(chunks)}] ✗{timeout_marker} {result.duration:.2f}s{pending_str} | {chunk_len} chars | {speed:.0f} chars/s | active: {active} | queue: {queue_size} | {result.error}")
            if debug:
